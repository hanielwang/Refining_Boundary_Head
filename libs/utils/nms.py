...
class SoftNMSop(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, segs, scores, cls_idxs_verb, cls_idxs_noun,
        ...
        sorted_cls_idxs_verb = cls_idxs_verb[inds]
        sorted_cls_idxs_verb = sorted_cls_idxs_verb[:n_segs]
        sorted_cls_idxs_noun = cls_idxs_noun[inds]
        sorted_cls_idxs_noun = sorted_cls_idxs_noun[:n_segs]
        return sorted_segs.clone(), sorted_scores.clone(), sorted_cls_idxs_verb.clone(), sorted_cls_idxs_noun.clone()
...
def batched_nms(
    ...
    if num_segs == 0:
        return torch.zeros([0, 2]),\
               torch.zeros([0,]),\
               torch.zeros([0,], dtype=cls_idxs_verb.dtype),\
               torch.zeros([0,], dtype=cls_idxs_noun.dtype)

    if multiclass:
        # multiclass nms: apply nms on each class independently
        new_segs, new_scores, new_cls_idxs_verb, new_cls_idxs_noun = [], [], [], []



        label_v_n = [str(v)+','+str(n) for v in range(0,97) for n in range(0,300)]
        label_v_n_idx = range(0,len(label_v_n))

        label_action_dict = dict(zip(label_v_n, label_v_n_idx))
        label_action_dict_opp = dict(zip(label_v_n_idx, label_v_n))


        cls_idxs_action = []
        for i in range(len(cls_idxs_verb)):
            action_cls = str(int(cls_idxs_verb[i].cpu())) + ',' + str(int(cls_idxs_noun[i].cpu()))
            cls_idxs_action.append(label_action_dict[action_cls])
        cls_idxs_action = torch.Tensor(cls_idxs_action)

        for class_id in torch.unique(cls_idxs_action):
            curr_indices = torch.where(cls_idxs_action == class_id)[0]
            # soft_nms vs nms
            if use_soft_nms:
                sorted_segs, sorted_scores, sorted_cls_idxs_verb, sorted_cls_idxs_noun = SoftNMSop.apply(
                    segs[curr_indices],
                    scores[curr_indices],
                    cls_idxs_verb[curr_indices],
                    cls_idxs_noun[curr_indices],
                    iou_threshold,
                    sigma,
                    min_score,
                    2,
            ...
            new_cls_idxs_verb.append(sorted_cls_idxs_verb)
            new_cls_idxs_noun.append(sorted_cls_idxs_noun)


        # cat the results
        new_segs = torch.cat(new_segs)
        new_scores = torch.cat(new_scores)
        new_cls_idxs_verb = torch.cat(new_cls_idxs_verb)
        new_cls_idxs_noun = torch.cat(new_cls_idxs_noun)

    else:
        # class agnostic

        if use_soft_nms:
            new_segs, new_scores, new_cls_idxs_verb, new_cls_idxs_noun = SoftNMSop.apply(
                segs, scores, cls_idxs_verb, cls_idxs_noun, iou_threshold,
        ...
        # new_cls_idxs_verb = new_cls_idxs_verb
        # new_cls_idxs_noun = new_cls_idxs_noun
    ...
    new_cls_idxs_verb = new_cls_idxs_verb[idxs[:max_seg_num]]
    new_cls_idxs_noun = new_cls_idxs_noun[idxs[:max_seg_num]]
    return new_segs, new_scores, new_cls_idxs_verb, new_cls_idxs_noun
