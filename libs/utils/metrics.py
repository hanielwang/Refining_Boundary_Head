...
class ANETdetection(object):
    ...
    def evaluate(self, preds, verbose=True):
        ...
        if isinstance(preds, pd.DataFrame):
            ...
            elif self.label_task == 'label_id_noun':
                # print('8888888888888888888888888')
                # print(preds['label_noun'])
                preds = pd.DataFrame({
                    'video-id' : preds['video-id'],
                    't-start' : preds['t-start'],#.tolist(),
                    't-end': preds['t-end'],#.tolist(),
                    'label': preds['label_noun'],#.tolist(),
                    'score': preds['score']#.tolist()
                })
        # always reset ap
        self.ap = None
        ...
        if verbose:
            # print the results
            print('[RESULTS] Action detection results on {:s}.'.format(self.label_task))
...
def compute_average_precision_detection(
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        tiou_arr = segment_iou(this_pred[['t-start', 't-end']].values,
                               this_gt[['t-start', 't-end']].values)  # one pred with multiple gts
        # print('----------------------tiou_arr----------------')
        # print(tiou_arr)

        # We would like to retrieve the predictions with highest tiou score.
        tiou_sorted_idx = tiou_arr.argsort()[::-1] # the index for large to small
        # print('----------------------tiou_sorted_idx----------------')
        # print(tiou_sorted_idx)
